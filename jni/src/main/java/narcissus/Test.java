...
public class Test {
    ...
    public static void main(String[] args) throws Exception {
        ...
        final Field ucpField = Narcissus.findField(classLoader.getClass(), "ucp");
        final Object ucpVal = Narcissus.getFieldVal(classLoader, ucpField);

        final Method getURLsMethod = Narcissus.findMethod(ucpVal.getClass(), "getURLs");
        for (URL url : (URL[]) Narcissus.callMethod(ucpVal, getURLsMethod)) {
            System.out.println("Classpath URL: " + url);
        }

        Cls cls = new Cls();
        Method triple = Narcissus.findMethod(cls.getClass(), "triple", int.class);
